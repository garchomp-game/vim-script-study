...
echoは、空白で複数連結することができる。

:echo range(5) range(7)

ただし、echoの場合は後ろにコメントをつけることはできない

なお、echomsgコマンドを実行したものをmessagesコマンドで
一覧表示できるようだが…どうやらデフォルトで
echoメッセージ含めた一覧が表示されているっぽい？

echomsgが受け付けるのは文字列と数値だけ。
そのため、リスト等を渡すとエラーになる。

echomsgは、エスケープ文字もそのまま出力する

### 改行の取り扱い

echoコマンドは、出力に先立って、改行を行う
:echo "1st" | echo "2nd"

改行しない場合は、echonコマンドを使う

:echo "1st" | echon "2nd"

### executeについて

executeコマンドは、引数を文字列をExコマンドとして実行するExコマンドです。
...
silentは、画面の出力を抑制する
ただし、silentではエラーメッセージまでは抑制しない。
エラーメッセージ込みで抑制するにはsilent!を使う。
が、基本的にsilent!の使用はあまり推奨されない。

### normal

ノーマルは、コマンドラインやvim script上でノーマル(アクション)を実行する
